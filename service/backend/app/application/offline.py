...
def _evaluate_row(row: dict[str, str]) -> dict[str, object]:
    ...
    vision_result = _mock_vision_result(label_id)
    rag_result = _mock_rag_result(label_id)
    vlm_result = _mock_vlm_result(label_id)
    decision = review_tool_outputs(ts_result, vision_result, vlm_result, rag_result)
    return {
        "sample_id": row.get("sample_id", ""),
        "target_label_id": label_id,
        "ts_label_id": ts_result.label_id,
        "vision_label_id": vision_result.label_id,
...
def _mock_vision_result(label_id: int) -> VisionResult:
    probabilities = {str(idx): 0.025 for idx in range(5)}
    probabilities[str(label_id)] = 0.90
    return VisionResult(
        model_name="offline_mock_vision",
        model_version="pre_model_mock",
        label_id=label_id,
        label_name=label_name(label_id),
        confidence=0.90,
        probabilities=probabilities,
        evidence={"phase_uniformity_score": 0.2, "ood_score": 0.1},
    )
...
def _mock_rag_result(label_id: int) -> RagResult:
    return RagResult(
        retriever_name="offline_mock_rag",
        retriever_version="pre_vector_db_mock",
        query=f"offline label_id={label_id}",
        documents=[
            RagDocument(
                document_id=f"offline-rule-{label_id}",
                title=f"{label_name(label_id)} rule",
                source="offline_mock",
                excerpt="오프라인 mock 평가를 위한 결정론적 RAG 근거입니다.",
                relevance=0.9,
            )
        ],
    )
...
def _summarize(rows: list[dict[str, object]]) -> OfflineEvaluationSummary:
    ...
    agreements = sum(
        1
        for row in rows
        if row["ts_label_id"] == row["vision_label_id"] == row["vlm_label_id"]
    )
    return OfflineEvaluationSummary(
        ...
        rejected=rejected,
        evidence_agreement_rate=agreements / total if total else 0.0,
    )
