...
from service.backend.app.application.offline import run_offline_mock_evaluation
...
def test_offline_mock_evaluation_writes_jsonl_and_summary(tmp_path: Path) -> None:
    ...
    assert summary.evidence_agreement_rate == 1.0
    assert rows[0]["status"] == "completed"
    assert rows[0]["vision_label_id"] == rows[0]["target_label_id"]
    assert rows[0]["rag_document_count"] == 1
