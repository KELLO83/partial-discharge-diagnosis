...
```powershell
python ml/vlm/train.py --model-profile smolvlm2_2b_qlora --manifest data/manifest.csv --sample-size 500 --max-steps 100
```
...
```text
smolvlm2_2b_qlora
  Current low-VRAM baseline profile. 4-bit QLoRA, frozen vision tower, frozen projector, 512x512 image budget.

qwen2_5_vl_3b_qlora
  Stable fallback profile when Qwen3-VL support is unstable or quality comparison is needed.

qwen3_vl_2b_qlora
  Future Qwen3-VL 2B comparison profile.
```
